...
class Fund_Initiate(unittest.TestCase):
    ...
    def choose_noncash(self):
        driver = self.driver
        Transactions = driver.find_element_by_link_text("Transactions")
        ActionChains(driver).move_to_element(Transactions).perform()
        time.sleep(2)
        Non_cash = driver.find_element_by_link_text("Non Cash Transactions")
        ActionChains(driver).click(Non_cash).perform()
    ...
    def test_noncash_AccrualAmounts(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.choose_noncash()
            self.listenNoParam()
            file_path = r"..\..\data\test_Pre_compile_iPortfolio\noncash_AccrualAmounts.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                value = dl[line].split(',')
                if value[1] != "ATO Tax Account Accruals" :
                    print(value[1])
                    time.sleep(1)
                    categorys = driver.find_element_by_id("ctl00_ContentPlaceHolder1_ddlCategory")
                    categorys.find_element_by_xpath("//option[@value='36']").click()
                    time.sleep(1)
                    types = driver.find_element_by_id("ctl00_ContentPlaceHolder1_ddlSubCategory")
            ...
            if self.i <=2:
                self.test_noncash_AccrualAmounts()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_noncash_AccrualAmounts"))

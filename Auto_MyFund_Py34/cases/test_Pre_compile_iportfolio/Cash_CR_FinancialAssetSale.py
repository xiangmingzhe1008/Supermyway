...
class Fund_Initiate(unittest.TestCase):
    ...
    def test_cr_financialassetsale(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.loadingpage()
            time.sleep(1)
            file_path =  r"..\..\data\test_Pre_compile_iPortfolio\cr_financialassetsale.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                ...
                categorys.find_element_by_xpath("//option[@value='2']").click()
                time.sleep(1)
            ...
            if self.i <=2:
                self.test_cr_financialassetsale()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_cr_financialassetsale"))

...
class Fund_Initiate(unittest.TestCase):
    ...
    def test_dr_corporateactions(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.loadingpage()
            time.sleep(1)
            file_path =  r"..\..\data\test_Pre_compile_iPortfolio\dr_corporateactions.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                ...
                thispath = "//option[@value='"+str(self.dr_option_values[2])+"']"
            ...
            if self.i <=2:
                self.test_dr_corporateactions()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_dr_corporateactions"))

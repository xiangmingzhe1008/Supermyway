...
class Fund_Initiate(unittest.TestCase):
    ...
    dr_option_values = [20,6,34,8,1,3,31,12,13,23]
        
    i = 1    
    
    def test_dr_financialassetpurchase(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.loadingpage()
            time.sleep(1)
            driver.find_element_by_xpath(".//*[@id='ctl00_ContentPlaceHolder1_gvTransaction']/tbody/tr[3]/td[2]").click()
            self.loadingpage()
            time.sleep(1)
            file_path =  r"..\..\data\test_Pre_compile_iPortfolio\dr_financialassetpurchase.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                ...
                thispath = "//option[@value='"+str(self.dr_option_values[4])+"']"
                categorys.find_element_by_xpath(thispath).click()
                time.sleep(1)
                option = line+2
                xpath = "html/body/form/div[3]/div[2]/div[2]/div[2]/div[1]/div[2]/div[2]/div[2]/div/table/tbody/tr[6]/td[3]/select/option["+str(option)+"]"
            ...
            if self.i <=2:
                self.test_dr_financialassetpurchase()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_dr_financialassetpurchase"))

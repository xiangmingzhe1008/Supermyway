...
class Fund_Initiate(unittest.TestCase):
    ...
    def test_noncash_InSpecieMemberPayments(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            for line in range(counts):
                ...
                categorys.find_element_by_xpath("//option[@value='38']").click()
                time.sleep(1)
            ...
            if self.i <=2:
                self.test_noncash_InSpecieMemberPayments()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_noncash_InSpecieMemberPayments"))

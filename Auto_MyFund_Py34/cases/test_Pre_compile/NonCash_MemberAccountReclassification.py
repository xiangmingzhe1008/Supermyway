...
class Fund_Initiate(unittest.TestCase):
    ...
    def test_noncash_MemberAccountReclassification(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.choose_noncash()
            self.listenNoParam()
            file_path = r"..\..\data\test_Pre_compile\noncash_MemberAccountReclassification.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                value = dl[line].split(',')
                if value[1] != "Pension Account Reclassification":
                    print(value[1])
                    time.sleep(1)
                    categorys = driver.find_element_by_id("ctl00_ContentPlaceHolder1_ddlCategory")
                    categorys.find_element_by_xpath("//option[@value='37']").click()
                    time.sleep(1)
            ...
            if self.i <=2:
                self.test_noncash_MemberAccountReclassification()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_noncash_MemberAccountReclassification"))

...
class Fund_Initiate(unittest.TestCase):
    ...
    def test_noncash_InternalContributionSplit(self):
        try:
            self.i += 1
            driver = self.driver
            self.login()
#             driver.find_element_by_id("ctl00_ContentPlaceHolder1_btnViewFund").click()
            ...
            self.choose_noncash()
            self.listenNoParam()
            file_path = r"..\..\data\test_Pre_compile\noncash_InternalContributionSplit.csv"
            data = open(file_path)
            ...
            for line in range(counts):
                ...
                categorys.find_element_by_xpath("//option[@value='18']").click()
                time.sleep(1)
            ...
            if self.i <=2:
                self.test_noncash_InSpecieRollovers()
            else:     
                pass
            
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Fund_Initiate("test_noncash_InternalContributionSplit"))

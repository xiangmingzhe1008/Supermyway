...
class Autorun(unittest.TestCase):
    ...
    file_name = "CMC CHESS Transactions File"
    print(file_name)  
    i = 1            
    def test_CMC_CHESS_Transactions_File(self):
        try:
            self.i = self.i+1
            driver = self.driver
            self.login()
            ...
            if self.i <=2:
                self.test_CMC_CHESS_Transactions_File()
            else:     
                pass
        
if __name__ == '__main__':
    suite = unittest.TestSuite()
    suite.addTest(Autorun("test_CMC_CHESS_Transactions_File"))

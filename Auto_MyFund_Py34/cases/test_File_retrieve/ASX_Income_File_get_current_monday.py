...
class Autorun(unittest.TestCase):
    ...
    file_name = "ASX Income File"   
    print(file_name)              
    i = 1
    def test_ASX_Income_File(self): 
        try:            
            self.i = self.i + 1
            driver = self.driver
            ...
            time.sleep(0.5)
            driver.find_element_by_id("ctl00_ContentPlaceHolder1_ddlFreq").send_keys("Weekly")
            time.sleep(0.5)
            driver.find_element_by_id('ctl00_ContentPlaceHolder1_ddlPeriod').send_keys('Other Period')
            Date = get_current_Monday.get_current_monday()             
            From_Date = Date[0]
            To_Date = Date[1]
#             print(From_Date,'           ',To_Date)
            time.sleep(0.5)
            driver.find_element_by_id('ctl00_ContentPlaceHolder1_calFrom_dateTextBox').send_keys(From_Date)
            time.sleep(0.5)
            driver.find_element_by_id('ctl00_ContentPlaceHolder1_calTo_dateTextBox').send_keys(To_Date)
            time.sleep(0.5)
            ...
            if self.i <=2:
                self.test_ASX_Income_File()
            else:     
                pass
               
if __name__ == '__main__':
    suite = unittest.TestSuite()        
    suite.addTest(Autorun("test_ASX_Income_File"))

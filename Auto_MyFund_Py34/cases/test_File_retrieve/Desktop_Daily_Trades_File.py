...
class Autorun(unittest.TestCase):
    ...
    file_name = "Bell Direct/Desktop Daily Trades File"   
    print(file_name)              
    i = 1
    def test_Desktop_Daily_Trades_File(self): 
        try:            
            self.i = self.i + 1
            driver = self.driver
            ...
            pic_name = "Bell Direct_Desktop Daily Trades File" + current_time + '.png'
            window_capture(pic_name)
            if self.i <=2:
                self.test_Desktop_Daily_Trades_File()
            else:     
                pass
               
if __name__ == '__main__':
    suite = unittest.TestSuite()        
    suite.addTest(Autorun("test_Desktop_Daily_Trades_File"))
